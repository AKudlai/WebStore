    ...
    public class EmailOrderProcessor : IOrderProcessor
    {
        ...
        public void ProcessOrder(Cart cart, ShippingDetails shippingInfo)
        {
            using (var smtpClient = new SmtpClient())
            {
                ...
                StringBuilder body = new StringBuilder().AppendLine("A new order has been submitted").AppendLine("---")
                    .AppendLine("Items:");
                ...
                body.AppendFormat("Total order value: {0:c}", cart.ComputeTotalValue()).AppendLine("---")
                    ...
                    .AppendLine(shippingInfo.Line2 ?? string.Empty).AppendLine(shippingInfo.Line3 ?? string.Empty)
                    .AppendLine(shippingInfo.City).AppendLine(shippingInfo.State ?? string.Empty).AppendLine(shippingInfo.Country)
                    .AppendLine(shippingInfo.Zip).AppendLine("---").AppendFormat(
                        "Gift wrap: {0}",
